...
# ============================
import os
TOKEN = os.environ.get("7790448792:AAHWkN5E2dA30IJlb9CULHydqRo8CjwZr6k"", "")
ADMIN_IDS = []
...
class Database:
    ...
    def add_fine(self, user_id: int, violation: str, amount: int, admin_id: int = None) -> int:
        ...
        conn.close()
        # Скрываем анкету при наличии штрафа
...
class BotApp:
    ...
    def load_admins(self):
        global ADMIN_IDS
        admins = self.db._fetchall("SELECT user_id, admin_level FROM users WHERE role = 'admin'")
    ...
    def register_handlers(self):
        ...
        self.bot.message_handler(commands=['anon_info'])(self.cmd_anon_info)
        ...
        self.bot.message_handler(commands=['lang'])(self.cmd_lang)
        self.bot.message_handler(commands=['settings'])(self.cmd_lang)
        self.bot.message_handler(commands=['admin'])(self.cmd_admin)
        self.bot.message_handler(commands=['admins'])(self.cmd_admin)
    ...
    def cmd_start(self, message):
        ...
        if unpaid > 0:
            msg = self.get_unpaid_fines_message(user_id) or f"У вас есть неоплаченный штраф на сумму {unpaid} Stars."
            self.bot.send_message(user_id, msg, reply_markup=self.get_fine_pay_keyboard())
            return
        ...
        if not user['registered']:
            if not user['has_subscribed']:
                ...
                markup.add(types.InlineKeyboardButton("Начать знакомство", callback_data="check_subscription"))
                self.bot.send_message(user_id, welcome, reply_markup=markup)
                return
            self.bot.set_state(user_id, UserStates.reg_language)
            with self.bot.retrieve_data(user_id) as st:
                st['flow'] = 'registration'
    ...
    def show_language_selection(self, user_id):
        ...
        markup.add(
            ...
            types.InlineKeyboardButton("🇧🇦 Башҡортса", callback_data="set_lang_ba")
        )
        self.bot.send_message(user_id, "🌐 Выберите язык интерфейса / Интерфейс телеһен һайлағыҙ:", reply_markup=markup)
    ...
    def get_tags_display(self, user):
        if not user.get('tags'):
            return "Не выбраны"
        tag_map = {tid: name for tid, name, _ in self.get_all_tags()}
        try:
            selected = [int(x) for x in user['tags'].split(',') if x]
            return ', '.join([tag_map.get(tid, '') for tid in selected if tag_map.get(tid)])
        except:
            return user['tags']
    ...
    def cmd_profile(self, message):
        ...
        tags_text = self.get_tags_display(user)

        text = "✉️ ИНФОРМАЦИЯ ДЛЯ ПОИСКА\n\n"
        text += "📸 Фото\n\n"
        ...
        text += f"🛠️ ИНФОРМАЦИЯ ДЛЯ АДМИНИСТРАЦИИ, Пользователя (Скрыто/Служебное)\n\n"
    ...
    def cmd_anon(self, message):
        ...
        if session:
            self.bot.send_message(user_id, f"У вас уже есть активный чат (сессия #{session['id']}). Напишите сообщение или завершите чат.")
    ...
    def cmd_anon_info(self, message):
        user_id = message.from_user.id
        if not self.is_admin(user_id):
            return
        parts = message.text.split()
        if len(parts) < 2:
            self.bot.send_message(user_id, "Использование: /anon_info ID_сессии")
            return
        try:
            session_id = int(parts[1])
        except:
            self.bot.send_message(user_id, "ID сессии должен быть числом.")
            return
        session = self.db._fetchone("SELECT * FROM anonymous_chat_sessions WHERE id = ?", (session_id,))
        if not session:
        ...
        for uid in [session['user1_id'], session['user2_id']]:
            ...
            if u:
                self.bot.send_message(user_id, f"Пользователь сессии #{session_id}:\nID: {u['user_id_str']}\nИмя: {u['name']}\nВозраст: {u['age']}\nГород: {u['city']}\nUsername: @{u.get('username') or 'нет'}")
            else:
                self.bot.send_message(user_id, f"Пользователь {uid} не найден.")
    ...
    def cmd_shop(self, message):
        ...
        text += "№ Название        Цена ⭐   Цена ₽   Экономия\n"
        text += "1 👑 1 день        9 ⭐      9 ₽      🔥\n"
        text += "2 👑 1 неделя     49 ⭐     49 ₽      🔥\n"
        text += "3 👑 3 месяца    199 ⭐    199 ₽      🔥\n"
        text += "4 👑 6 месяцев   349 ⭐    349 ₽      🔥\n"
        text += "5 👑 12 месяцев  599 ⭐    599 ₽      🔥\n"
        text += "6 💎 Пополнить баланс\n"
        text += "7 🚀 Ускоренная верификация - 150 ⭐\n"
        text += "8 📝 Дополнительные черновики - 100 ⭐"
        ...
        buttons = [
            ...
            types.InlineKeyboardButton("6", callback_data="shop_topup"),
            types.InlineKeyboardButton("7", callback_data="shop_verif_fast"),
            types.InlineKeyboardButton("8", callback_data="shop_drafts")
        ]
        markup.add(*buttons)
    ...
    def cmd_lang(self, message):
        user_id = message.from_user.id
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("Русский", callback_data="set_lang_ru"),
            types.InlineKeyboardButton("Башҡорт", callback_data="set_lang_ba")
        )
        self.bot.send_message(user_id, "Выберите язык / Тел һайлағыҙ:", reply_markup=markup)
    ...
    def cmd_admin(self, message):
        ...
        if not self.is_admin(user_id):
            ...
            return
        stats = self.get_admin_stats()
        markup = self.get_admin_menu(user_id)
    ...
    def show_profile_for_confirm(self, user_id):
        ...
        tags_text = self.get_tags_display(user)
        text = "✅ Профиль создан!\n\n"
        text += f"🆔 Ваш ID: {user['user_id_str']}\n"
        text += f"👤 Имя: {user['name']}\n"
        text += f"📅 Возраст: {user['age']} ({user.get('category', '18-100')})\n"
        text += f"📍 Город: {user['city']}\n"
        text += f"📝 О себе: {user['about']}\n"
        text += f"🎯 Интересы: {user.get('interests', '')}\n"
        text += f"🏷 Теги: {tags_text}\n"
        text += f"⚥ Пол: {gender_text}\n\n"
        text += "Сохраните ваш ID — он понадобится для обращений в поддержку!\n\nВсё верно?"
        markup = types.InlineKeyboardMarkup(row_width=2)
        markup.add(
            types.InlineKeyboardButton("✅ ВСЁ ВЕРНО", callback_data="confirm_reg_yes"),
    ...
    def get_unpaid_fines_message(self, user_id):
        fines = self.db.get_unpaid_fines(user_id)
        total = sum(f['amount'] for f in fines)
        balance = self.db.get_balance(user_id)
        if not fines:
            return None
        latest = fines[-1]
        issued = latest['issued_date'][:10]
        text = "🚨 ВАС ОШТРАФОВАЛИ!\n\n"
        text += f"Нарушение: {latest['violation']}\n"
        text += f"Сумма штрафа: {latest['amount']} ⭐ Stars\n"
        text += f"Дата выдачи: {issued}\n\n"
        text += "⚡ Последствия:\n"
        ...
        text += "(Анкета не отображается его у других)\n\n"
        text += f"Ваш баланс: {balance} ⭐ Stars\n"
        text += f"Не хватает: {max(0, total - balance)} ⭐ Stars"
        return text
    ...
    def get_admin_level(self, user_id):
        ...
        if user_id not in ADMIN_IDS:
            return 0
        return int(user.get('admin_level') or 1)

    def get_admin_menu(self, user_id):
        level = self.get_admin_level(user_id)
        markup = types.InlineKeyboardMarkup(row_width=1)

        # 1 уровень
        markup.add(types.InlineKeyboardButton("🛡️ 1 уровень", callback_data="admin_level_1"))
        # 2 уровень
        if level >= 2:
            markup.add(types.InlineKeyboardButton("🔧 2 уровень", callback_data="admin_level_2"))
        # 3 уровень
        if level >= 3:
            markup.add(types.InlineKeyboardButton("💎 3 уровень", callback_data="admin_level_3"))
        return markup

    def get_admin_level_menu(self, level):
        markup = types.InlineKeyboardMarkup(row_width=1)
        if level == 1:
            buttons = [
                ("🚫 Выдать бан", "admin_ban"),
                ("✅ Снять бан", "admin_unban"),
                ("💰 Выдать штраф", "admin_fine"),
                ("❌ Снять штраф", "admin_remove_fine"),
                ("🏷️ Добавить тег администратора", "admin_add_tag"),
                ("🚫 Убрать тег администратора", "admin_remove_tag"),
                ("📹 Проверить верификацию", "admin_verification_pending"),
                ("🔙 Назад", "admin_back")
            ]
        elif level == 2:
            buttons = [
                ("⬇️ Снять администратора", "admin_remove_admin"),
                ("⬆️ Назначить администратора", "admin_add_admin"),
                ("📋 Логи действий администрации", "admin_logs"),
                ("🔙 Назад", "admin_back")
            ]
        elif level == 3:
            buttons = [
                ("⭐ Выдать звёзды пользователю", "admin_give_stars"),
                ("🔄 Обнулить звёзды у пользователя", "admin_set_stars"),
                ("👑 Выдать премию", "admin_give_premium"),
                ("❌ Отозвать премию", "admin_remove_premium"),
                ("🔙 Назад", "admin_back")
            ]
        else:
            buttons = [("🔙 Назад", "admin_back")]
    ...
    def show_next_profile(self, user_id):
        ...
        query = "SELECT * FROM users WHERE visible = 1 AND status NOT IN ('blocked', 'hidden') AND registered = 1 AND verified = 1 AND user_id != ?"
        params = [user_id]
        # Категория по возрасту
        if user.get('category'):
            query += " AND category = ?"
            params.append(user['category'])
        # Возраст из профиля или фильтров
        age_min = filters.get('age_min') or user.get('search_age_min')
        age_max = filters.get('age_max') or user.get('search_age_max')
        if age_min:
            query += " AND age >= ?"
        ...
        # Город из профиля или фильтров
        city = filters.get('city') or user.get('search_city')
        if city:
            query += " AND city = ?"
            params.append(city)
        # Пол из профиля или фильтров
        gender = filters.get('gender') or (user.get('search_gender') if user.get('search_gender') != 'all' else None)
        if gender:
            query += " AND gender = ?"
    ...
    def send_profile(self, user_id, target):
        ...
        tags_text = self.get_tags_display(target)
        text = "📸 Фото\n\n"
        text += f"ID аккаунта: {target['user_id_str']}\n"
        text += f"👤 Имя: {target['name']}\n"
        text += f"🎂 Возраст: {target['age']} лет (Категория: {target.get('category', '18-100')})\n"
        text += f"🏙️ Город: {target['city']}\n\n"
        text += f"🎯 Увлечения: {tags_text}\n\n"
        text += f"📝 О себе: {target['about']}"
    ...
    def start_verification(self, user_id):
        text = "🔐 Верификация аккаунта\n\n"
        text += "Для подтверждения, что вы реальный человек, пройдите верификацию.\n\n"
        text += "Инструкция:\n"
        text += "1️⃣ Откройте камеру телефона\n"
        text += "2️⃣ Запишите кружок (видео-сообщение) где вы говорите слово: «РусскиеЗнакомства»\n"
        text += "3️⃣ В кружке вы должны быть видны чётко\n"
        text += "4️⃣ Отправьте кружок сюда\n\n"
        text += "⚠️ Важно: Без верификации вы НЕ сможете пользоваться ботом!"
        self.bot.send_message(user_id, text)
    ...
    def start_anon_search(self, user_id):
        ...
        if len(self.anon_waiting) >= 2:
            ...
            session_id = self.db.create_chat_session(user1, user2)
            self.bot.send_message(user1, f"Чат найден! Вы можете общаться анонимно.\nID сессии: {session_id}")
            self.bot.send_message(user2, f"Чат найден! Вы можете общаться анонимно.\nID сессии: {session_id}")
    ...
    def handle_callback(self, call):
        ...
        if data == "check_subscription":
            try:
                ...
                self.db.update_user(user_id, has_subscribed=1)
                self.bot.answer_callback_query(call.id, "Проверка пропущена в тестовом режиме.")
                self.cmd_start(call.message)
            return

        # Смена языка
        if data == "set_lang_ru":
            self.db.update_user(user_id, language='ru')
            self.bot.answer_callback_query(call.id, "Язык изменён на русский.")
            with self.bot.retrieve_data(user_id) as st:
            ...
            return
        if data == "set_lang_ba":
            self.db.update_user(user_id, language='ba')
            self.bot.answer_callback_query(call.id, "Тел башҡорт теленә алыштырылды.")
            with self.bot.retrieve_data(user_id) as st:
                if st and st.get('flow') == 'registration':
                    self.start_registration(user_id)
                    return
        ...
        if data == "confirm_reg_yes":
            ...
            self.start_verification(user_id)
            return
    ...
    def handle_admin_callback(self, call):
        ...
        if not self.is_admin(admin_id):
            ...
            return
        data = call.data
        level = self.get_admin_level(admin_id)

        if data == "admin_level_1":
            self.bot.send_message(admin_id, "🛡️ Меню 1 уровня", reply_markup=self.get_admin_level_menu(1))
            return
        elif data == "admin_level_2":
            if level < 2:
                self.bot.answer_callback_query(call.id, "Нет доступа", show_alert=True)
                return
            self.bot.send_message(admin_id, "🔧 Меню 2 уровня", reply_markup=self.get_admin_level_menu(2))
            return
        elif data == "admin_level_3":
            if level < 3:
                self.bot.answer_callback_query(call.id, "Нет доступа", show_alert=True)
                return
            self.bot.send_message(admin_id, "💎 Меню 3 уровня", reply_markup=self.get_admin_level_menu(3))
            return

        # Проверка уровней для кнопок
        level1_actions = {'admin_ban', 'admin_unban', 'admin_fine', 'admin_remove_fine', 'admin_add_tag', 'admin_remove_tag', 'admin_verification_pending'}
        level2_actions = {'admin_remove_admin', 'admin_add_admin', 'admin_logs', 'admin_broadcast'}
        level3_actions = {'admin_give_stars', 'admin_set_stars', 'admin_give_premium', 'admin_remove_premium'}

        base_action = data.split('_next')[0] if data.endswith('_next') else data
        if base_action in level1_actions and level < 1:
            self.bot.answer_callback_query(call.id, "Нет доступа", show_alert=True)
            return
        if base_action in level2_actions and level < 2:
            self.bot.answer_callback_query(call.id, "Нет доступа", show_alert=True)
            return
        if base_action in level3_actions and level < 3:
            self.bot.answer_callback_query(call.id, "Нет доступа", show_alert=True)
            return

        if data == "admin_verification_pending":
            self.admin_verification_pending_handler(call)
        elif data == "admin_ban":
            self.bot.set_state(admin_id, UserStates.admin_ban)
            self.bot.send_message(admin_id, "Введите ID пользователя для бана:")
        elif data == "admin_unban":
            self.bot.set_state(admin_id, UserStates.admin_unban)
            self.bot.send_message(admin_id, "Введите ID пользователя для разбана:")
        elif data == "admin_fine":
            self.bot.set_state(admin_id, UserStates.admin_fine_issue)
            self.bot.send_message(admin_id, "Введите: ID_пользователя сумма причина")
        elif data == "admin_remove_fine":
            self.bot.set_state(admin_id, UserStates.admin_remove_fine)
            self.bot.send_message(admin_id, "Введите ID пользователя для снятия штрафа:")
        elif data == "admin_add_tag":
            self.bot.set_state(admin_id, UserStates.admin_add_admin)
            self.bot.send_message(admin_id, "Введите ID пользователя для добавления тега модератора (уровень 1). Например: #12345")
        elif data == "admin_remove_tag":
            self.bot.set_state(admin_id, UserStates.admin_remove_admin)
            self.bot.send_message(admin_id, "Введите ID пользователя для удаления тега модератора:")
            ...
            self.bot.set_state(admin_id, UserStates.admin_add_admin)
            self.bot.send_message(admin_id, "Введите ID пользователя для назначения администратором и уровень (1-3) через пробел. Например: #12345 2")
        elif data == "admin_remove_admin":
            self.bot.set_state(admin_id, UserStates.admin_remove_admin)
            self.bot.send_message(admin_id, "Введите ID пользователя для снятия администратора:")
        elif data == "admin_logs":
            self.cmd_logs(call.message)
        elif data == "admin_give_stars":
            self.admin_give_stars_handler(call)
        elif data == "admin_set_stars":
            self.bot.set_state(admin_id, UserStates.admin_set_stars)
            self.bot.send_message(admin_id, "Введите ID пользователя и сумму для обнуления/установки баланса через пробел:")
        elif data == "admin_give_premium":
            self.bot.set_state(admin_id, UserStates.admin_give_premium)
            self.bot.send_message(admin_id, "Введите ID пользователя и количество дней Premium:")
        elif data == "admin_remove_premium":
            self.bot.set_state(admin_id, UserStates.admin_remove_premium)
            self.bot.send_message(admin_id, "Введите ID пользователя для отзыва Premium:")
    ...
    def admin_verification_action(self, call):
        ...
        if action == "approve":
            ...
            text = "✅ Поздравляем! Верификация пройдена!\n\n"
            text += "🎉 Ваш аккаунт подтверждён!\n\n"
            text += "Теперь вам доступны все функции бота:\n"
            text += "· 🔍 Поиск анкет\n"
            text += "· ❤️ Лайки и дизлайки\n"
            text += "· 💬 Анонимный чат\n"
            text += "· 📂 Черновики\n"
            text += "· и другие\n\n"
            text += "У вас появилась галочка ✅ у имени в анкете!\n\n"
            text += "Бонус: +15 Stars за верификацию!"
            markup = types.InlineKeyboardMarkup()
    ...
    def admin_verification_reject_handler(self, message):
        ...
        if user['status'] == 'blocked':
            ...
            text += "Вы исчерпали 3 попытки верификации.\n\n"
            text += "Ваш аккаунт заблокирован до решения модератора.\n\n"
            text += "Для восстановления обратитесь в поддержку.\n\n"
            text += f"Ваш ID: {user['user_id_str']}"
        else:
            text = "❌ Верификация отклонена\n\n"
            text += f"Причина: {reason}\n"
            text += f"Попытка: {user['verification_attempts']}/3\n\n"
            text += "Что делать:\n"
            text += "1️⃣ Запишите новый кружок\n"
            text += "2️⃣ Чётко произнесите слово «РусскиеЗнакомства»\n"
            text += "3️⃣ Отправьте его снова"
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("📸 ОТПРАВИТЬ НОВЫЙ КРУЖОК", callback_data="send_new_video_note"))
            self.bot.send_message(user_id, text, reply_markup=markup)
    ...
    def start_fine_payment(self, user_id):
        ...
        if self.db.deduct_stars(user_id, total, "Оплата штрафа"):
            ...
            self.db.update_user(user_id, visible=1)
            text = "✅ Штраф оплачен!\n\n"
            text += f"Списано: {total} Stars\n"
            text += f"Остаток: {self.db.get_balance(user_id)} Stars\n\n"
            text += "Все ограничения сняты! 🎉"
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔍 НАЧАТЬ ПОИСК", callback_data="start_search"))
            self.bot.send_message(user_id, text, reply_markup=markup)
        else:
    ...
    def video_note_handler(self, message):
        ...
        if message.video_note:
            ...
            self.bot.send_message(user_id, "✅ Кружок получен!\n\nВаша верификация отправлена модераторам на проверку.\n\n⏳ Ожидайте, проверка займёт до 24 часов.\n\nСтатус: НА РАССМОТРЕНИИ")
        else:
    ...
    def admin_add_admin_handler(self, message):
        ...
        parts = text.split()
        if len(parts) < 2:
            self.bot.send_message(user_id, "Использование: ID_пользователя уровень (1-3). Например: #12345 2")
            return
        ...
        try:
            ...
            self.bot.send_message(user_id, "Уровень должен быть от 1 до 3.")
            return
        self.db.update_user(target['user_id'], role='admin', admin_level=level)
        self.db.add_admin_log(user_id, "Назначение администратора", target['user_id'], f"{parts[0]} уровень {level}")
        self.bot.send_message(user_id, f"Пользователь {parts[0]} назначен администратором {level} уровня.")
        if target['user_id'] not in ADMIN_IDS:
            ADMIN_IDS.append(target['user_id'])
        self.bot.set_state(user_id, None)
